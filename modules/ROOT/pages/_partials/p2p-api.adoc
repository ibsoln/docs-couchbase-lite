...
Use `disableTLS` to disable TLS communication if necessary
...
NOTE: If `disableTLS` is true, TLS communication is disabled and the `tlsIdentity` property is ignored.
So, the URL scheme used to connect to the listener will be `ws://` NOT `wss://`.
...
// tag::config-tls-id[]
Use this configuration property to specify the TLS Identity used in TLS communication.
