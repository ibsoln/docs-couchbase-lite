...
Use the `{url-api-reference-urlendpointconfiguration-disable-tls}`  property of `{url-api-class-urlendpointconfiguration}` to disable TLS communication if necessary

This property works in conjunction with `TLSidentity`, which enables developers to define the key and-or certificate to be used.

* If `disableTLS` is true -- TLS communication is disabled and the `{url-api-reference-urlendpointconfiguration-tls-id}` property is ignored.
* If `disableTLS` is false or not specified -- TLS communication is enabled.
Set the required `{url-api-reference-urlendpointconfiguration-tls-id}` property.
Use the `wss://` URL scheme used to connect to the listener.
...
API Reference:  {url-api-reference-urlendpointconfiguration-disable-tls}
...
Use the
{url-api-reference-urlendpointconfiguration-tls-id} property of
{url-api-class-urlendpointconfiguration} to configure the TLS Identity used in TLS communication.

If `TLSIdentity` is not set when TLS is enabled (default), the listener uses an auto-generated anonymous self-signed identity.
This doesn't authenticate or verify trust (on the client side) but it does encrypt communication, giving a more secure option than Non-TLS communication.

*Resuable Id* -- If the platform supports secure storage, you can save and re-use the auto-generated identity. This avoids the need to generate new identities every time.
...
API Reference:  {url-api-reference-urlendpointconfiguration-tls-id}
...
// tag::config-auth[]
Use this configuration property to specify the authenticator to be used by the listener to authenticate the request.
...
// tag::config-auth-default[]
API Reference:  {url-api-reference-urlendpointconfiguration-auth}
...
Use the `ListenerPasswordAuthenticator` class to provide password type authentication such as the Basic Authentication.

The actual authentication logic is provided through a `ListenerPasswordAuthenticatorDelegate` object provided by the application code.
Often in the form of a closure where the platform permits.
...
Use the `ListenerCertificateAuthenticator` class to authenticate  clients certificates.
This requires you have TLS enabled.
You can verify the certificate using either:

* Root CA certificate chains -- any client cert signed by the given root CA cert chains is authenticated
...
Initialize the `ListenerCertificateAuthenticator` using the `ListenerCertificateAuthenticatorDelegate` interface. This identifies the application logic to be used to verify a certificate chain received from the client.

The logic should take the form of function or block (such as, a closure expression) if the platform allows.
