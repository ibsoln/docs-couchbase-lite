...
Use the `{url-api-references-urlendpointconfiguration-disable-tls}`  parameter to `{url-api-class-urlendpointconfiguration}` to disable TLS communication if necessary
...
* If `disableTLS` is true -- TLS communication is disabled and `{url-api-references-urlendpointconfiguration-tls-id}` is ignored.
* If `disableTLS` is false or not specified -- TLS communication is enabled.
+
Set the required `{url-api-references-urlendpointconfiguration-tls-id}` and use the `wss://` URL scheme used to connect to the listener.
...
Use the
{url-api-references-urlendpointconfiguration-tls-id} parameter of
...
Initialize the `ListenerCertificateAuthenticator` using the `ListenerCertificateAuthenticatorDelegate` interface.
This identifies the application logic to be used to verify a certificate chain received from the client.
