...
ifeval::[{param-platform} == "android"]
:securestorage: Keystore
:securestorage-api: KeyChain API
endif::[]

ifeval::[{param-platform} == "ios"]
...
This content provides sample code covering the implementation of peer-to-peer sync over websockets. Specifically it covers the implementation of an {glos-term-active-peer}, that will sync database changes with a connected {glos-term-passive-peer}.
...
Use the `.selfSignedCert` to skip listener cert validation. -- see <<configure-server-authentication>>

<.> The `authenticator` mode is configured to use basic authentication.
Other supported options are `ClientCertificateAuthenticator` if you want to use client cert based authentication -- see <<configure-client-authentication>>
...
Use the
{url-api-class-replicator-config} class to initialize the replication configuration with the
...
* Use the {url-api-prop-replicator-config-target} parameter to specify the passive peer's URL including the port the listener is listening on.
The URL scheme for web socket URLs uses `ws:` (non-TLS) or `wss:` (SSL/TLS) prefixes.

* Use the
{url-api-prop-replicator-config-database} parameter to specify the local database to be synced.
...
Use `{url-api-class-replicator-config}`, with its {url-api-prop-replicator-config-rep-type} and
...
Use the {url-api-prop-replicator-config-auth} property object to provide client authentication credentials to the replicator.

Valid options include:

* Use the `{url-api-class-replicator-config-basic-auth}` parameter to set up basic authentication with the username and password credentials.

* Use the `{url-api-class-replicator-config-cert-auth}` parameter to configure the client TLS certs to be presented to the server, on connection. This applies only to the {url-api-class-urlendpointlistener}.
...
This example shows client cert authentication using an identity from the {securestorage}.
...
For how to import identities and add them to the {securestorage} see {xref-cbl-pg-p2p-manage-tls-id}

== Initiate Sync

Use `{url-api-class-replicator-config}`, with its {url-api-constructor-replicator-init-config} parameter, to specify the configuration set you have defined. You may also optionally add a change listener (see <<monitor-sync>>) before starting the replicator running using {url-api-method-replicator-start}.
...
The default conflict resolver is applied (see {xref-cbl-pg-conflict-auto}) unless you use the {url-api-prop-replicator-config-conflict} parameter to specify a custom conflict resolver object (see: {xref-cbl-pg-conflict-custom}).
