...
To compare the peer-to-peer context with the typical client-server communication model, the:
* Active peer is analogous to a client,
* Passive peer is analogous to a server.
...
<.> Use the {url-api-class-replicator-config} class's {url-api-constructor-replicator-config-db-tgt} constructor to initialize the replicator configuration with the local database -- see <<configure-target>>
...
+
use {url-api-prop-replicator-config-AcceptOnlySelfSignedServerCertificate}(true) to accept ONLY self-signed certificates.
Validation of the certificate is bypassed -- see: <<listener-authentication>>

<.> The `authenticator` is configured to use basic authentication.
...
----
<.> Note use of TLS (`wss://`) prefix to URI to ensure TLS encrypted communication(recommended).
====
...
Use the `{url-api-class-replicator-config-cert-auth}` to configure the client TLS certificates to be presented to the server, on connection. This applies only to the {url-api-class-urlendpointlistener}.

The server will verify the certificate -- checking the chain of trust -- *before* offering the certificate to the listener’s authenticator.
