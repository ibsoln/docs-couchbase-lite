...
----
include::{snippet-p2psync-ws}[tags="listener-initialize;!listener-config-tls-id-caCert;!listener-config-tls-id-SelfSigned;!listener-config-tls-id-set;!listener-config-auth-cert", indent=0]
----
...
<.> Validate the user credentials (`thisUser` and `thisPassword`), where `username` and `password` are the valid credentials and `thisUser` and `thisPassword` are the presented credentials-- see <<client-authentication>> for more
...
include::{snippet-p2psync-ws}[tags=listener-config-auth-pwd, indent=0]

----
<.> Where 'thisUser' and 'thisPassword' are the previously populated values to be authenticated.
====
// include::{snippet}[tags=listener-config-auth-pwd-full, indent=0]
...
include::{snippet}[tags=listener-config-auth-cert-root, indent=0]
--
+
Application Logic::
+
...
include::{snippet-p2psync-ws}[tags=deleteTlsIdentity, indent=0]
