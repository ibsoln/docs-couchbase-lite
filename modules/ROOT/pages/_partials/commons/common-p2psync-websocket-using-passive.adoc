...
----
include::{snippet-p2psync-ws}[tags="listener-initialize;!listener-config-tls-enable;!listener-config-tls-id-anon;!listener-config-tls-id-caCert;!listener-config-tls-id-SelfSigned;!listener-config-client-auth-root;!listener-config-tls-id-set;!listener-config-client-root-ca;!listener-config-client-auth-self-signed", indent=0]

----

ifeval::["{param-platform}"=="{platform-android}"]
<.> You will need to include this database initializer in the application's `onCreate()` method.
endif::[]
...
It is *not* advisable to switch it off in production.
This setting is ignored when _Basic Authentication_ is required; but must be 'false' when _Client Cert Authentication_ is required -- see also:  <<tls-security>>
<.> Apply basic authentication to validate the user credentials (`thisUser` and `thisPassword`), where `username` and `password` are the valid credentials and `thisUser` and `thisPassword` are the presented credentials-- see <<client-authentication>> for more
...
<.> Store the generated identity as alias `couchbase-demo-cert`.
