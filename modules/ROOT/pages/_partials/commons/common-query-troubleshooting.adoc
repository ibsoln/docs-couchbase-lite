// BEGIN: common-query-troubleshooting.adoc
// USED-IN: <source-language>-troubleshooting.adoc
// Applies-to: All platforms, except Javascript

// DO NOT EDIT
// Only include this if the inclusion is used as a full page, rather than as a component
ifdef::param-fullpage[]
ifndef::param-abstract[:param-abstract: This content describes how to use Couchbase Lite on {param-title} Query's diagnostic command `explain` to build better queries.]
...
== Query Explain
=== Using
The `<query>.explain()` command provides useful insight to help you diagnose query performance issues and optimize your queries.
You can embed the command call inside your app, or use it interactively within a `cblite` shell. to examine how your query is working, see: <<use-qe>> for how to use it in either scenario.
...
let thisQuery = QueryBuilder
    .select(
        SelectResult.property("domains"))
    .from(DataSource.database(database))
    .where(Expression.property("type").equalTo(Expression.string("university")))
    .groupBy(Expression.property("country").ascending())
    .orderBy(Ordering.property("name").ascending())  // <.>
print(try thisQuery.explain()) <.>
----
<.> Construct your query as normal
<.> Call the query's `explain` method; output will be sent to the app's log file.
--
...
=== Output
The output from `explain()` remains the same whether invoked by an app, or `cblite` -- see <<qe-output>> for an example of how it looks.
...
let hotelSearchQuery = QueryBuilder
      ...
      .where(
        _Property.TYPE.like(Expression.string("%hotel%")) // <.>
      .and(_Property.CITY.like(Expression.string("%\(locationStr)%")))
      )
----
...
For example, you can observe a similar situation to that shown in <<use-like-based-queries>> when using the `LOWER` function on an indexed property.

.Query
----
query --explain {"WHERE":["=",["LOWER()",[".type"]],"hotel"],"WHAT":[["."]]}
----
...
.Query
----
Query --explain {"WHERE":["=",[".type"],"hotel"],"WHAT":[["."]]}
----
...
Although, note that using query offsets becomes increasingly less effective as the overhead of skipping a growing number of rows each time increases. You can work around this, by instead using ranges of searxh-key values. If the last search-key value of batch one was 'x' then that could become the starting point for your next batch and-so-on.
