...
// tag::conflict-custom-resolution-examples[]
[{tabs}]
====
Local Wins::
+
...
--
====
// end::conflict-custom-resolution-examples[]
...
.Points of Note:
* Within the conflict handlefr, you can modify the `document` parameter which is the same instance of `Document` that is passed to the `save()` method.
